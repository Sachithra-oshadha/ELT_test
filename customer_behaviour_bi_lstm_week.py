...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'customer_behavior_bilstm_week_{now}.log'),
...
class ElectricityDataset(Dataset):
    def __init__(self, data: np.ndarray, sequence_length: int, prediction_horizon: int = 672):
        self.data = data
        self.sequence_length = sequence_length
        self.prediction_horizon = prediction_horizon  # Number of timesteps in a week (7 days * 24 hours * 4 intervals)
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # Predict avg_import_kw for the next week (sequence of 672 timesteps)
        y = self.data[idx + self.sequence_length:idx + self.sequence_length + self.prediction_horizon, 0]
        return torch.FloatTensor(x), torch.FloatTensor(y)

class BiLSTM(nn.Module):
    def __init__(self, input_size: int, hidden_size: int = 64, num_layers: int = 2, dropout: float = 0.2, output_length: int = 672):
        ...
        self.fc = nn.Linear(hidden_size * 2, output_length)  # Output sequence of length 672
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = self.fc(out[:, -1, :])  # Take the last time step and map to output sequence
        return out

class CustomerBehaviorPipeline:
    def __init__(self, output_base_dir: str = f"customer_outputs_bilstm_week_{now}"):
        self.db_config = {
            'dbname': os.getenv('DB_NAME_BILSTM_WEEK'),
    ...
    def load_existing_model(self, customer_ref: int) -> tuple[nn.Module, float, float]:
        try:
            ...
            if result:
                ...
                model = BiLSTM(input_size=8, output_length=672)
    ...
    def predict_next_timestep(self, model: nn.Module, last_sequence: np.ndarray, 
        ...
        try:
            ...
            predictions = predictions.cpu().numpy()  # Shape: (1, 672)
            # Create dummy array with 672 rows to match prediction length
            dummy_array = np.zeros((predictions.shape[1], len(scaler.mean_)))
            dummy_array[:, 0] = predictions[0]  # Assign 672 predictions to avg_import_kw column
            predictions_transformed = scaler.inverse_transform(dummy_array)[:, 0]
            predictions_transformed = np.maximum(0, predictions_transformed)
            logger.info(f"Predicted weekly avg_import_kw sequence: mean {np.mean(predictions_transformed):.4f} kW")
