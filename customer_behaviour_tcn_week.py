...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'customer_behavior_tcn_week_{now}.log'),
...
class ElectricityDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        y = self.data[idx + self.sequence_length]  # Predicting all features
...
class TCNN(nn.Module):
    def __init__(self, input_size: int, num_channels: List[int], kernel_size: int = 3):
        ...
        self.fc = nn.Linear(num_channels[-1], input_size)  # Output all features
...
class CustomerBehaviorPipeline:
    def __init__(self, output_base_dir: str = "customer_outputs"):
        """Initialize the pipeline with database configuration and TCNN parameters."""
        self.db_config = {
            'dbname': os.getenv('DB_NAME_TCN_WEEK'),
        ...
        try:
            ...
            with torch.no_grad():
                for batch_x, batch_y in val_loader:
                    batch_x, batch_y = batch_x.to(device), batch_y.to(device)
                    outputs = model(batch_x)
                    predictions.extend(outputs.cpu().numpy()[:, 0])  # Only for avg_import_kw for R2
                    actuals.extend(batch_y.cpu().numpy()[:, 0])
    ...
    def predict_next_week(self, model: nn.Module, last_sequence: np.ndarray, 
                         scaler: StandardScaler, sequence_length: int, num_predictions: int = 672) -> np.ndarray:
        """Predict the next week's features (672 timesteps at 15-minute intervals)."""
        try:
            model.eval()
            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
            model = model.to(device)
            ...
            predictions = []
            current_sequence = last_sequence.copy()  # Shape: (sequence_length, features)
            ...
            for _ in range(num_predictions):
                ...
                prediction_value = prediction.cpu().numpy()[0]  # All features
                # Transform prediction back to original scale
                prediction_transformed = scaler.inverse_transform(prediction_value.reshape(1, -1))[0]
                prediction_transformed = np.maximum(0, prediction_transformed)  # Ensure non-negative
                predictions.append(prediction_transformed)
                
                # Update sequence with predicted values
                current_sequence = np.vstack((current_sequence[1:], prediction_value))
            
            predictions = np.array(predictions)  # Shape: (num_predictions, num_features)
            logger.info(f"Predicted {predictions.shape[1]} features for next week: {len(predictions)} timesteps")
            return predictions
        except Exception as e:
            logger.error(f"Failed to make week-long prediction: {e}")
            raise

    def create_prediction_plot(self, df: pd.DataFrame, predictions: np.ndarray, customer_ref: int, sequence_length: int):
        """Create and save a plot of 30 days of historical data and week-long prediction for avg_import_kw."""
        try:
            output_dir = os.path.join(self.output_base_dir, f"customer_{customer_ref}")
            os.makedirs(output_dir, exist_ok=True)
            
            plt.figure(figsize=(15, 7))
            # Get last 30 days of data (2880 timesteps)
            past_n_days = 30 * 24 * 4  # 30 days * 24 hours * 4 (15-min intervals)
            last_n = df.tail(past_n_days)
            timestamps = last_n['timestamp']
            actual_kw = last_n['avg_import_kw']
            
            last_timestamp = timestamps.iloc[-1]
            time_delta = timedelta(minutes=15)
            future_timestamps = [last_timestamp + (i + 1) * time_delta for i in range(len(predictions))]
            extended_timestamps = pd.concat([pd.Series(timestamps), pd.Series(future_timestamps)], ignore_index=True)
            extended_kw = np.concatenate([actual_kw.values, predictions[:, 0]])  # Only avg_import_kw
            
            plt.plot(extended_timestamps, extended_kw, 'b-', label='Actual + Predicted avg_import_kw')
            plt.axvline(x=last_timestamp, color='r', linestyle='--', label='Prediction Start')
            plt.title(f'Customer {customer_ref} - Last 30 Days and Next Week Prediction for avg_import_kw')
            ...
            output_path = os.path.join(output_dir, f'week_prediction_plot_{customer_ref}_{datetime.now(pytz.timezone('Asia/Kolkata')).strftime("%Y%m%d_%H%M%S")}.png')
            plt.savefig(output_path, bbox_inches='tight')
            plt.close()
            logger.info(f"Saved week-long prediction plot at: {output_path}")
            return output_path
        except Exception as e:
            logger.error(f"Failed to create week-long prediction plot: {e}")
            raise
    ...
    def process_customer(self, customer_ref: int, sequence_length: int = 96, batch_size: int = 32) -> Tuple[np.ndarray, str]:
        """Process a single customer: fetch data, train model, predict for one week, and save plot."""
        try:
            ...
            # Predict next week (672 timesteps)
            last_sequence = scaled_data[-sequence_length:]
            predictions = self.predict_next_week(model, last_sequence, scaler, sequence_length)
            ...
            for customer_ref in customer_refs:
                ...
                if predictions is not None and plot_path is not None:
                    ...
                    logger.info(f"Completed processing for customer {customer_ref}. "
                               f"Predicted {len(predictions)} timesteps, Plot: {plot_path}")
                else:
...
if __name__ == "__main__":
    ...
    for result in results:
        logger.info(f"Customer {result['customer_ref']}: Predicted avg_import_kw for next week: {result['predictions'][:5, 0].tolist()}... (first 5 values)")
